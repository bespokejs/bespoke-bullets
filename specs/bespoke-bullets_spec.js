(function() {
	...
	describe("bespoke-bullet", function() {
		...
		var createDeck = function(hasBulletsOnSecondSlide) {
				slides = [];
				...
				for (var i = 0; i < NO_OF_SLIDES; i++) {
					...
					if (!hasBulletsOnSecondSlide && i === 1) {
						...
						slides[i].innerHTML = '' +
							'<ul>' +
								'<li data-bespoke-bullet>Bullet</li>' +
								'<li data-bespoke-bullet>Bullet</li>' +
								'<li data-bespoke-bullet>Bullet</li>' +
							'</ul>';
					}
			...
			getBullet = function(deck, slideIndex, bulletIndex) {
				return deck.slides[slideIndex].querySelector('[data-bespoke-bullet]:nth-child(' + (bulletIndex + 1) + ')');
			};
